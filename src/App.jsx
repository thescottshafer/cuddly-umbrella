...
export default function App() {
  ...
  const [showRCEditor, setShowRCEditor] = useState(false); const [rcEditTarget, setRcEditTarget] = useState(null);
  ...
  const rcEntry = useMemo(()=>rateCard.find(p=>p.program===npProg),[npProg]);
  const rcMs = useMemo(()=>rcEntry?.milestones||[],[rcEntry]);
  const selMs = useMemo(()=>rcMs.find(m=>m.name===npMs),[rcMs,npMs]);

  const customers = useMemo(()=>[...new Set(rateCard.map(p=>p.customer))].sort(),[]);
  ...
  return (
    <div style={{fontFamily:"'DM Sans',system-ui,sans-serif",minHeight:"100vh",background:"#f8f7f4",color:"#1a1a18"}}>
      ...
      <div style={{background:"#1a1a18",color:"#f8f7f4",padding:"0 24px",display:"flex",alignItems:"center",gap:16,height:52}}>
        ...
        <button onClick={()=>{setRcEditTarget(null);setShowRCEditor(true);}} style={{fontSize:12,padding:"5px 12px",border:"1px solid #3a3a36",borderRadius:6,background:"transparent",color:"#c8c5be",cursor:"pointer"}}>Rate Cards ({rateCard.length})</button>
      ...
      <div style={{padding:"20px 24px",maxWidth:1440,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",gap:2,marginBottom:16,borderBottom:"1px solid #dddbd4"}}>
          {[["projects","Projects"],["resources","Resource View"],["rdb","RDB Reconciliation"],["revenue","Revenue Forecast"],["actual","Actual Revenue"],["pay","Pay Tracker"]].map(([id,lbl])=>(
        ...
        {tab==="pay"&&<PayView projects={projects} roster={roster} updTask={updTask}/>}
      </div>
      ...
      {showRCEditor&&(
        <RateCardEditor
          ...
          editTarget={rcEditTarget}
          setEditTarget={setRcEditTarget}
          onClose={()=>{setShowRCEditor(false);setRcEditTarget(null);}}
        />
      )}
      ...
      {showNP&&(
        <Modal onClose={()=>setShowNP(false)} title="Create New Project" w={620}>
          <div style={{padding:"16px 20px",display:"flex",flexDirection:"column",gap:12}}>
            ...
            <Sel value={npProg} onChange={v=>{setNpProg(v);}} opts={rateCard.map(p=>({v:p.program,l:`${p.customer} / ${p.program}`}))} ph="-- Select program --"/>
  ...
  return (
    <div className="hr" style={{display:"grid",gridTemplateColumns:"26px 1fr 52px 170px 115px 78px 90px 145px 105px",
      ...
      <div style={{fontSize:11,color:"#888780"}}>
        {/* Pay type toggle -- only on production tasks */}
        {(taskType==="production"||taskType==="fielding"||taskType==="permitting") ? (
          <div style={{display:"flex",flexDirection:"column",gap:3}}>
            <div style={{display:"flex",gap:3}}>
              {[["hourly","$/hr"],["unit","$/unit"]].map(([v,l])=>(
                <button key={v} onClick={()=>updTask(pid,t.id,"payType",v)}
                  style={{flex:1,padding:"2px 4px",fontSize:9,fontWeight:600,border:"1px solid",cursor:"pointer",borderRadius:4,
                    borderColor:(t.payType||"hourly")===v?"#185FA5":"#ccc9c0",
                    background:(t.payType||"hourly")===v?"#E6F1FB":"#f8f7f4",
                    color:(t.payType||"hourly")===v?"#185FA5":"#888780"}}>{l}</button>
              ))}
            </div>
            {(t.payType||"hourly")==="unit" ? (
              <div>
                <input type="number" step="0.01"
                  value={t.unitRate??""} placeholder="$/unit"
                  onChange={e=>updTask(pid,t.id,"unitRate",e.target.value===""?null:parseFloat(e.target.value))}
                  style={{width:"100%",height:24,padding:"0 5px",border:"0.5px solid #185FA5",borderRadius:5,
                    background:"#E6F1FB",fontSize:11,color:"#1a1a18",outline:"none"}}/>
                <div style={{fontSize:9,color:"#888780",marginTop:1}}>
                  {t.unitRate&&p.uForecast?("Est: $"+(t.unitRate*p.uForecast).toFixed(0)):("set rate")}
                </div>
              </div>
            ) : (
              <div style={{textAlign:"right"}}>
                ...
                <div style={{fontSize:10}}>${t.eeRate?.toFixed(2)||"-"}/hr</div>
              </div>
            )}
          </div>
        ) : (
...
// -- RATE CARD EDITOR ---------------------------------------------------------
function RateCardEditor({rateCard, setRateCard, editTarget, setEditTarget, onClose}) {
  // view: "list" | "program" (editing one program)
  const [view, setView] = useState("list");
  const [prog, setProg] = useState(null); // the program being edited

  // wizard state for creating/editing a program
  const [wcStep, setWcStep] = useState(1);   // 1=basics, 2=milestones, 3=tasks
  const [wcCust, setWcCust] = useState("");
  const [wcProg, setWcProg] = useState("");
  const [wcMilestones, setWcMilestones] = useState([
    {name:"", masterRate:"", milestonePct:"", uom:"Passing", w2ProdTarget:"0.35", subProdTarget:"0.70", tasks:[]}
  ]);
  const [wcMsIdx, setWcMsIdx] = useState(0); // which milestone we are editing tasks for

  const customers = useMemo(()=>[...new Set(rateCard.map(p=>p.customer))].sort(),[rateCard]);

  function openNew() {
    setWcStep(1); setWcCust(""); setWcProg("");
    setWcMilestones([{name:"",masterRate:"",milestonePct:"",uom:"Passing",w2ProdTarget:"0.35",subProdTarget:"0.70",tasks:[]}]);
    setWcMsIdx(0); setView("program"); setProg(null);
  }

  function openEdit(p) {
    setWcCust(p.customer); setWcProg(p.program);
    setWcMilestones(p.milestones.map(m=>({
      ...m,
      masterRate: String(m.masterRate),
      milestonePct: String(m.milestonePct),
      w2ProdTarget: String(m.w2ProdTarget),
      subProdTarget: String(m.subProdTarget),
      tasks: m.tasks.map(t=>({...t, revPct: String(t.revPct), templateEeRate: String(t.templateEeRate), unitRate: String(t.unitRate||""), payType: t.payType||"hourly"}))
    })));
    setWcMsIdx(0); setWcStep(1); setView("program"); setProg(p);
  }

  function delProgram(p) {
    if (!window.confirm("Delete program " + p.program + "?")) return;
    setRateCard(prev=>prev.filter(r=>!(r.customer===p.customer&&r.program===p.program)));
  }

  function addMilestone() {
    setWcMilestones(prev=>[...prev,{name:"",masterRate:"",milestonePct:"",uom:"Passing",w2ProdTarget:"0.35",subProdTarget:"0.70",tasks:[]}]);
    setWcMsIdx(wcMilestones.length);
  }

  function updMs(idx, field, val) {
    setWcMilestones(prev=>prev.map((m,i)=>i===idx?{...m,[field]:val}:m));
  }

  function delMs(idx) {
    if (wcMilestones.length===1) return;
    setWcMilestones(prev=>prev.filter((_,i)=>i!==idx));
    setWcMsIdx(Math.max(0, idx-1));
  }

  function addTask(msIdx) {
    setWcMilestones(prev=>prev.map((m,i)=>i!==msIdx?m:{
      ...m, tasks:[...m.tasks,{name:"",revPct:"",rdbTask:"",templateEeRate:"40",payType:"hourly",unitRate:""}]
    }));
  }

  function updTask(msIdx, tIdx, field, val) {
    setWcMilestones(prev=>prev.map((m,i)=>i!==msIdx?m:{
      ...m, tasks: m.tasks.map((t,j)=>j!==tIdx?t:{...t,[field]:val})
    }));
  }

  function delTask(msIdx, tIdx) {
    setWcMilestones(prev=>prev.map((m,i)=>i!==msIdx?m:{
  ...
  function saveProgram() {
    if (!wcCust.trim()||!wcProg.trim()) return;
    const milestones = wcMilestones.map(m=>({
      name: m.name.trim(),
      masterRate: parseFloat(m.masterRate)||0,
      milestonePct: parseFloat(m.milestonePct)||0,
      milestoneRate: (parseFloat(m.masterRate)||0)*(parseFloat(m.milestonePct)||0),
      uom: m.uom||"Passing",
      w2ProdTarget: parseFloat(m.w2ProdTarget)||0.35,
      subProdTarget: parseFloat(m.subProdTarget)||0.70,
      tasks: m.tasks.filter(t=>t.name.trim()).map(t=>({
        name: t.name.trim(),
        revPct: parseFloat(t.revPct)||0,
        rdbTask: t.rdbTask||"",
        templateEeRate: parseFloat(t.templateEeRate)||40,
        payType: t.payType||"hourly",
        unitRate: t.payType==="unit"?(parseFloat(t.unitRate)||0):null,
      }))
    }));
    const entry = {customer:wcCust.trim(), program:wcProg.trim(), milestones};
    if (prog) {
      setRateCard(prev=>prev.map(p=>p.customer===prog.customer&&p.program===prog.program?entry:p));
    } else {
      setRateCard(prev=>[...prev, entry]);
    }
    setView("list");
  }

  const pctTotal = wcMilestones.reduce((s,m)=>s+(parseFloat(m.milestonePct)||0),0);
  const msTasks = wcMilestones[wcMsIdx]?.tasks||[];
  const msRevTotal = msTasks.reduce((s,t)=>s+(parseFloat(t.revPct)||0),0);

  const IS = (s={})=>({height:30,padding:"0 9px",border:"0.5px solid #ccc9c0",borderRadius:6,background:"#fff",fontSize:12,color:"#1a1a18",outline:"none",...s});

  return (
    <Modal onClose={onClose} title={view==="list"?"Rate Card Management":prog?"Edit Program: "+prog.program:"New Program"} w={860}>
      {view==="list" ? (
        <div>
          <div style={{padding:"12px 20px",borderBottom:"1px solid #eee",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{fontSize:12,color:"#888780"}}>{rateCard.length} programs across {customers.length} customers</div>
            <button onClick={openNew} style={{padding:"5px 16px",background:"#1a1a18",color:"#fff",border:"none",borderRadius:6,fontSize:12,cursor:"pointer",fontWeight:500}}>+ New Program</button>
          </div>
          <div style={{overflowY:"auto",maxHeight:480}}>
            {customers.map(cust=>(
              <div key={cust}>
                <div style={{padding:"8px 20px",background:"#f8f7f4",fontSize:11,fontWeight:600,color:"#555",textTransform:"uppercase",letterSpacing:"0.05em",borderBottom:"0.5px solid #eee"}}>{cust}</div>
                {rateCard.filter(p=>p.customer===cust).map(p=>(
                  <div key={p.program} style={{display:"flex",alignItems:"center",padding:"10px 20px",borderBottom:"0.5px solid #f0ede8",gap:12}}>
                    <div style={{flex:1}}>
                      <div style={{fontSize:13,fontWeight:500,color:"#1a1a18"}}>{p.program}</div>
                      <div style={{fontSize:11,color:"#888780",marginTop:2}}>{p.milestones.length} milestones * {p.milestones.reduce((s,m)=>s+m.tasks.length,0)} tasks total</div>
                    </div>
                    <div style={{fontSize:11,color:"#888780"}}>
                      {[...new Set(p.milestones.map(m=>m.uom))].join(" / ")}
                    </div>
                    <button onClick={()=>openEdit(p)} style={{padding:"4px 12px",fontSize:11,border:"1px solid #ccc9c0",borderRadius:5,background:"#fff",cursor:"pointer"}}>Edit</button>
                    <button onClick={()=>delProgram(p)} style={{padding:"4px 10px",fontSize:11,border:"1px solid #E24B4A",borderRadius:5,background:"#fff",color:"#E24B4A",cursor:"pointer"}}>Delete</button>
                  </div>
                ))}
              </div>
            ))}
        ...
        <div style={{padding:"16px 20px",display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{display:"flex",gap:4,marginBottom:4}}>
            {["1 - Basics","2 - Milestones","3 - Tasks"].map((s,i)=>(
              <div key={s} onClick={()=>setWcStep(i+1)} style={{padding:"4px 14px",borderRadius:20,fontSize:11,cursor:"pointer",fontWeight:500,
                background:wcStep===i+1?"#1a1a18":"#f0ede8",color:wcStep===i+1?"#fff":"#555"}}>{s}</div>
            ))}
          </div>
          ...
          {wcStep===1&&(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <div>
                  <label style={{fontSize:11,fontWeight:500,color:"#555",display:"block",marginBottom:3}}>Customer name *</label>
                  <input value={wcCust} onChange={e=>setWcCust(e.target.value)} placeholder="e.g. Brightspeed"
                    list="cust-list" style={IS({width:"100%"})}/>
                  <datalist id="cust-list">{customers.map(c=><option key={c} value={c}/>)}</datalist>
                </div>
                <div>
                  <label style={{fontSize:11,fontWeight:500,color:"#555",display:"block",marginBottom:3}}>Program name *</label>
                  <input value={wcProg} onChange={e=>setWcProg(e.target.value)} placeholder="e.g. Brightspeed FTTH ENG-TX"
                    style={IS({width:"100%"})}/>
                </div>
              </div>
              <div style={{background:"#f8f7f4",borderRadius:6,padding:"10px 12px",fontSize:12,color:"#555"}}>
                A program groups all billable milestones for one contract. Each milestone has a % of the master rate and its own set of tasks. You will define milestones and tasks in the next steps.
              </div>
              <div style={{display:"flex",justifyContent:"flex-end"}}>
                <button onClick={()=>setWcStep(2)} disabled={!wcCust.trim()||!wcProg.trim()}
                  style={{padding:"5px 20px",background:(!wcCust.trim()||!wcProg.trim())?"#ccc":"#1a1a18",color:"#fff",border:"none",borderRadius:6,fontSize:12,cursor:"pointer"}}>
                  Next: Milestones
                </button>
              </div>
            </div>
          ...
          {wcStep===2&&(
            <div style={{display:"flex",gap:12}}>
              ...
              <div style={{width:200,flexShrink:0}}>
                <div style={{fontSize:11,fontWeight:600,color:"#555",marginBottom:6,textTransform:"uppercase",letterSpacing:"0.04em"}}>Milestones</div>
                {wcMilestones.map((m,i)=>(
                  <div key={i} onClick={()=>setWcMsIdx(i)}
                    style={{padding:"7px 10px",borderRadius:6,marginBottom:4,cursor:"pointer",fontSize:12,
                      background:wcMsIdx===i?"#E6F1FB":"#f8f7f4",color:wcMsIdx===i?"#185FA5":"#333",
                      fontWeight:wcMsIdx===i?500:400,border:wcMsIdx===i?"1px solid #b3d4f0":"1px solid transparent"}}>
                    {m.name||"(unnamed)"}
                    <span style={{fontSize:10,color:"#aaa89e",marginLeft:4}}>{m.milestonePct?(Math.round(parseFloat(m.milestonePct)*100)+"%"):""}</span>
                  </div>
                ))}
                <button onClick={addMilestone} style={{width:"100%",padding:"5px",fontSize:11,border:"1px dashed #ccc9c0",borderRadius:6,background:"transparent",cursor:"pointer",color:"#888780",marginTop:4}}>
                  + Add milestone
                </button>
                <div style={{marginTop:8,fontSize:11,color:Math.abs(pctTotal-1)<0.001?"#3B6D11":"#A32D2D"}}>
                  Total: {Math.round(pctTotal*100)}% {Math.abs(pctTotal-1)>0.001?"(should = 100%)":""}
                </div>
              </div>
              {/* Milestone detail */}
              <div style={{flex:1,display:"flex",flexDirection:"column",gap:8}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 100px 100px 100px",gap:8}}>
                  <div>
                    <label style={{fontSize:11,color:"#555",display:"block",marginBottom:2}}>Milestone name *</label>
                    <input value={wcMilestones[wcMsIdx]?.name||""} onChange={e=>updMs(wcMsIdx,"name",e.target.value)} style={IS({width:"100%"})}/>
                  </div>
                  <div>
                    <label style={{fontSize:11,color:"#555",display:"block",marginBottom:2}}>Master rate</label>
                    <input type="number" value={wcMilestones[wcMsIdx]?.masterRate||""} onChange={e=>updMs(wcMsIdx,"masterRate",e.target.value)} placeholder="0.00" style={IS({width:"100%"})}/>
                  </div>
                  <div>
                    <label style={{fontSize:11,color:"#555",display:"block",marginBottom:2}}>MS % of master</label>
                    <input type="number" step="0.01" value={wcMilestones[wcMsIdx]?.milestonePct||""} onChange={e=>updMs(wcMsIdx,"milestonePct",e.target.value)} placeholder="0.00" style={IS({width:"100%"})}/>
                  </div>
                  <div>
                    <label style={{fontSize:11,color:"#555",display:"block",marginBottom:2}}>UOM</label>
                    <input value={wcMilestones[wcMsIdx]?.uom||""} onChange={e=>updMs(wcMsIdx,"uom",e.target.value)} placeholder="Passing" style={IS({width:"100%"})}/>
                  </div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  <div>
                    <label style={{fontSize:11,color:"#555",display:"block",marginBottom:2}}>W2 prod. target (default 0.35)</label>
                    <input type="number" step="0.01" value={wcMilestones[wcMsIdx]?.w2ProdTarget||""} onChange={e=>updMs(wcMsIdx,"w2ProdTarget",e.target.value)} style={IS({width:"100%"})}/>
                  </div>
                  <div>
                    <label style={{fontSize:11,color:"#555",display:"block",marginBottom:2}}>Sub prod. target (default 0.70)</label>
                    <input type="number" step="0.01" value={wcMilestones[wcMsIdx]?.subProdTarget||""} onChange={e=>updMs(wcMsIdx,"subProdTarget",e.target.value)} style={IS({width:"100%"})}/>
                  </div>
                </div>
                {wcMilestones[wcMsIdx]&&(
                  <div style={{fontSize:11,color:"#888780",background:"#f8f7f4",padding:"6px 10px",borderRadius:5}}>
                    Milestone rate = {parseFloat(wcMilestones[wcMsIdx].masterRate||0).toFixed(2)} x {(parseFloat(wcMilestones[wcMsIdx].milestonePct||0)*100).toFixed(0)}% = <strong>${(parseFloat(wcMilestones[wcMsIdx].masterRate||0)*parseFloat(wcMilestones[wcMsIdx].milestonePct||0)).toFixed(4)}</strong>
                  </div>
                )}
                {wcMilestones.length>1&&(
                  <button onClick={()=>delMs(wcMsIdx)} style={{alignSelf:"flex-start",padding:"3px 10px",fontSize:11,border:"1px solid #E24B4A",borderRadius:5,background:"#fff",color:"#E24B4A",cursor:"pointer"}}>
          ...
          {wcStep===2&&(
            <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
              <button onClick={()=>setWcStep(1)} style={{padding:"5px 14px",fontSize:12,border:"1px solid #ccc9c0",borderRadius:6,background:"#fff",cursor:"pointer"}}>Back</button>
              <button onClick={()=>setWcStep(3)} style={{padding:"5px 20px",background:"#1a1a18",color:"#fff",border:"none",borderRadius:6,fontSize:12,cursor:"pointer"}}>Next: Tasks</button>
            </div>
          )}
          ...
          {wcStep===3&&(
            <div style={{display:"flex",gap:12}}>
              ...
              <div style={{width:160,flexShrink:0}}>
                <div style={{fontSize:11,fontWeight:600,color:"#555",marginBottom:6,textTransform:"uppercase",letterSpacing:"0.04em"}}>Select Milestone</div>
                {wcMilestones.map((m,i)=>(
                  <div key={i} onClick={()=>setWcMsIdx(i)}
                    style={{padding:"6px 10px",borderRadius:6,marginBottom:4,cursor:"pointer",fontSize:12,
                      background:wcMsIdx===i?"#E6F1FB":"#f8f7f4",color:wcMsIdx===i?"#185FA5":"#333",
                      fontWeight:wcMsIdx===i?500:400,border:wcMsIdx===i?"1px solid #b3d4f0":"1px solid transparent"}}>
                    {m.name||"(unnamed)"}
                    <div style={{fontSize:10,color:"#aaa89e"}}>{m.tasks.length} tasks</div>
                  </div>
                ))}
              </div>
              {/* Task list */}
              <div style={{flex:1,overflowY:"auto",maxHeight:380}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  <div style={{fontSize:11,fontWeight:600,color:"#555",textTransform:"uppercase",letterSpacing:"0.04em"}}>
                    Tasks for: {wcMilestones[wcMsIdx]?.name||"(select milestone)"}
                  </div>
                  <div style={{fontSize:11,color:Math.abs(msRevTotal-1)<0.001?"#3B6D11":"#A32D2D"}}>
                    Rev% total: {Math.round(msRevTotal*100)}%
                  </div>
                </div>
                {/* Header row */}
                <div style={{display:"grid",gridTemplateColumns:"1fr 60px 1fr 70px 80px 90px 26px",gap:5,marginBottom:4}}>
                  {["Task Name","Rev%","RDB Task","EE Rate $","Pay Type","Unit Rate $",""].map(h=>(
                    <div key={h} style={{fontSize:10,color:"#aaa89e",fontWeight:500}}>{h}</div>
                  ))}
                </div>
                {msTasks.map((t,ti)=>(
                  <div key={ti} style={{display:"grid",gridTemplateColumns:"1fr 60px 1fr 70px 80px 90px 26px",gap:5,marginBottom:5,alignItems:"center"}}>
                    <input value={t.name} onChange={e=>updTask(wcMsIdx,ti,"name",e.target.value)} placeholder="Task name" style={IS({width:"100%"})}/>
                    <input type="number" step="0.01" value={t.revPct} onChange={e=>updTask(wcMsIdx,ti,"revPct",e.target.value)} placeholder="0.75" style={IS({width:"100%"})}/>
                    <input value={t.rdbTask||""} onChange={e=>updTask(wcMsIdx,ti,"rdbTask",e.target.value)} placeholder="RDB category/task" style={IS({width:"100%"})}/>
                    <input type="number" value={t.templateEeRate} onChange={e=>updTask(wcMsIdx,ti,"templateEeRate",e.target.value)} placeholder="40" style={IS({width:"100%"})}/>
                    {/* Pay type toggle - only for non-QC/PM tasks */}
                    <select value={t.payType||"hourly"} onChange={e=>updTask(wcMsIdx,ti,"payType",e.target.value)}
                      style={IS({width:"100%",background:t.payType==="unit"?"#fffbe6":"#fff",color:t.payType==="unit"?"#854F0B":"#1a1a18"})}>
                      <option value="hourly">Hourly</option>
                      <option value="unit">Unit Rate</option>
                    </select>
                    <input type="number" step="0.01" value={t.unitRate||""} onChange={e=>updTask(wcMsIdx,ti,"unitRate",e.target.value)}
                      placeholder="$/unit" disabled={t.payType!=="unit"}
                      style={IS({width:"100%",background:t.payType==="unit"?"#fffbe6":"#f8f7f4",color:t.payType==="unit"?"#1a1a18":"#aaa89e"})}/>
                    <button onClick={()=>delTask(wcMsIdx,ti)} style={{width:24,height:24,border:"none",background:"none",color:"#E24B4A",cursor:"pointer",fontSize:14,padding:0}}>x</button>
                  </div>
                ))}
                <button onClick={()=>addTask(wcMsIdx)} style={{padding:"4px 12px",fontSize:11,border:"1px dashed #ccc9c0",borderRadius:5,background:"transparent",cursor:"pointer",color:"#888780",marginTop:4}}>
                  + Add task
                </button>
                <div style={{marginTop:8,fontSize:11,color:"#888780",background:"#f8f7f4",padding:"6px 10px",borderRadius:5}}>
                  Pay Type: "Hourly" uses the loaded EE rate. "Unit Rate" is used for subcontractors or ICs paid per unit performed -- this flows into the Pay Tracker tab.
                </div>
              </div>
            </div>
          )}
          {wcStep===3&&(
            <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
              <button onClick={()=>setWcStep(2)} style={{padding:"5px 14px",fontSize:12,border:"1px solid #ccc9c0",borderRadius:6,background:"#fff",cursor:"pointer"}}>Back</button>
              <div style={{display:"flex",gap:8}}>
                <button onClick={()=>setView("list")} style={{padding:"5px 14px",fontSize:12,border:"1px solid #ccc9c0",borderRadius:6,background:"#fff",cursor:"pointer"}}>Cancel</button>
                <button onClick={saveProgram} disabled={!wcCust.trim()||!wcProg.trim()}
                  style={{padding:"5px 20px",background:"#1a1a18",color:"#fff",border:"none",borderRadius:6,fontSize:12,cursor:"pointer",fontWeight:500}}>
                  {prog?"Save Changes":"Create Program"}
                </button>
              </div>
            </div>
          )}
...
// -- PAY TRACKER VIEW ----------------------------------------------------------
function PayView({projects, roster, updTask}) {
  const [filterRes, setFilterRes] = useState("");
  const [filterType, setFilterType] = useState("all"); // all | employee | subcontractor

  // Build pay rows: one entry per task that has a resource, units, and payType="unit"
  const payRows = useMemo(()=>{
    ...
    projects.forEach(p=>{
      ...
      p.tasks.forEach(t=>{
        ...
        const emp = roster.find(r=>r.name===t.resource);
        // hourly pay row (always show completed tasks with rdbHours)
        if (t.rdbHours && t.status==="Complete") {
          const hourlyPay = (t.eeRate??t.templateEeRate??0)*t.rdbHours;
          rows.push({
            resource: t.resource,
            isSub: emp?.isSubcontractor||false,
            customer: p.customer,
            project: p.project||"",
            milestone: p.milestone,
            task: t.name,
            ...
            uom: p.uom||"",
            units: units||0,
            unitRate: t.unitRate||null,
            rdbHours: t.rdbHours||0,
            eeRate: t.eeRate||t.templateEeRate||0,
            status: t.status,
            // For unit-rate tasks: pay = units * unitRate
            // For hourly tasks: pay = rdbHours * eeRate
            payAmount: (t.payType==="unit" && t.unitRate && units)
              ? units * t.unitRate
              : (t.rdbHours||0)*(t.eeRate||t.templateEeRate||0),
            invoiceable: t.status==="Complete",
          });
        }
      });
    });
  ...
  const allResources = useMemo(()=>[...new Set(payRows.map(r=>r.resource))].sort(),[payRows]);

  const filtered = useMemo(()=>payRows.filter(r=>{
    if (filterRes && r.resource!==filterRes) return false;
    if (filterType==="employee" && r.isSub) return false;
    if (filterType==="subcontractor" && !r.isSub) return false;
    return true;
  }),[payRows,filterRes,filterType]);

  // Summary by resource
  const summary = useMemo(()=>{
    const map = {};
    filtered.forEach(r=>{
      if (!map[r.resource]) map[r.resource]={resource:r.resource,isSub:r.isSub,totalPay:0,invoiceablePay:0,hours:0,tasks:0};
      map[r.resource].totalPay += r.payAmount;
      map[r.resource].invoiceablePay += r.invoiceable?r.payAmount:0;
      map[r.resource].hours += r.rdbHours||0;
      map[r.resource].tasks++;
    });
    return Object.values(map).sort((a,b)=>b.totalPay-a.totalPay);
  },[filtered]);

  const totalPay = summary.reduce((s,r)=>s+r.totalPay,0);
  const totalInvoiceable = summary.reduce((s,r)=>s+r.invoiceablePay,0);
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:10,marginBottom:16}}>
        {[
          {l:"Total Pay Tracked", v:"$"+Math.round(totalPay).toLocaleString(), s:"all completed tasks"},
          {l:"Invoiceable Pay", v:"$"+Math.round(totalInvoiceable).toLocaleString(), s:"complete tasks"},
          {l:"Resources Active", v:summary.length, s:"with logged hours"},
          {l:"Unit Rate Tasks", v:filtered.filter(r=>r.payType==="unit").length, s:"non-hourly"},
        ].map(m=>(
          <div key={m.l} style={{background:"#fff",border:"0.5px solid #dddbd4",borderRadius:10,padding:"12px 14px"}}>
            <div style={{fontSize:10,color:"#888780",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:4}}>{m.l}</div>
            <div style={{fontSize:22,fontWeight:600,color:"#1a1a18"}}>{m.v}</div>
            <div style={{fontSize:11,color:"#aaa89e",marginTop:2}}>{m.s}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",marginBottom:16}}>
        <div style={{position:"relative"}}>
          <select value={filterRes} onChange={e=>setFilterRes(e.target.value)} style={IS({minWidth:200,paddingRight:24})}>
            <option value="">All resources</option>
            {allResources.map(r=><option key={r} value={r}>{r}</option>)}
          </select>
          <span style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",pointerEvents:"none",fontSize:10,color:"#888780"}}>v</span>
        </div>
        <div style={{fontSize:12,color:"#888780"}}>Show:</div>
        {[["all","All"],["employee","Employees"],["subcontractor","Subcontractors"]].map(([v,l])=>(
          <button key={v} onClick={()=>setFilterType(v)}
            style={{padding:"4px 12px",fontSize:12,border:"1px solid",cursor:"pointer",borderRadius:6,
              borderColor:filterType===v?"#185FA5":"#ccc9c0",
              background:filterType===v?"#E6F1FB":"#fff",
              color:filterType===v?"#185FA5":"#555",fontWeight:filterType===v?500:400}}>{l}</button>
        ))}
        <div style={{flex:1}}/>
        <div style={{fontSize:12,color:"#888780",fontStyle:"italic"}}>Showing completed tasks with RDB hours logged</div>
      </div>

      {/* Summary cards */}
      {summary.length===0&&(
        <div style={{textAlign:"center",padding:"3rem",color:"#aaa89e",fontSize:13}}>
          No pay data available yet. Assign resources to tasks, log RDB hours, and mark tasks Complete to see pay tracking here.
        </div>
      )}
      {summary.length>0&&(
        <>
          <div style={{fontSize:11,fontWeight:600,color:"#555",textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:8}}>Summary by Resource</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:10,marginBottom:20}}>
            {summary.map(s=>{
              const pct = s.totalPay>0?Math.round(s.invoiceablePay/s.totalPay*100):0;
              return (
                <div key={s.resource} style={{background:"#fff",border:"0.5px solid #dddbd4",borderRadius:10,padding:"12px 14px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                    <div>
                      <div style={{fontSize:13,fontWeight:600,color:"#1a1a18"}}>{s.resource}</div>
                      <div style={{fontSize:11,color:"#888780"}}>{s.tasks} task{s.tasks!==1?"s":""} * {s.hours.toFixed(1)}h</div>
                    </div>
                    {s.isSub&&<span style={{fontSize:10,padding:"2px 6px",borderRadius:3,background:"#FAEEDA",color:"#854F0B",fontWeight:600}}>SUB</span>}
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                    <span style={{fontSize:11,color:"#888780"}}>Total pay</span>
                    <span style={{fontSize:13,fontWeight:600,color:"#1a1a18"}}>${Math.round(s.totalPay).toLocaleString()}</span>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                    <span style={{fontSize:11,color:"#888780"}}>Invoiceable</span>
                    <span style={{fontSize:13,fontWeight:600,color:"#3B6D11"}}>${Math.round(s.invoiceablePay).toLocaleString()}</span>
                  </div>
                  <div style={{height:4,background:"#f0ede8",borderRadius:2}}>
                    <div style={{width:pct+"%",height:"100%",background:"#63991f",borderRadius:2}}/>
                  </div>
                </div>
              );
            })}
          ...
          <div style={{fontSize:11,fontWeight:600,color:"#555",textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:8}}>Task Detail</div>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            <thead><tr style={{background:"#f8f7f4",borderBottom:"1px solid #dddbd4"}}>
              {["Resource","Customer","Project","Milestone","Task","Status","Pay Type","Units/Hours","Rate","Pay"].map(h=>(
                <th key={h} style={{padding:"8px 10px",textAlign:"left",color:"#888780",fontWeight:500,whiteSpace:"nowrap"}}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {filtered.map((r,i)=>(
                <tr key={i} style={{borderBottom:"0.5px solid #f0ede8",background:r.invoiceable?"#fff":"#fafaf8"}}>
                  <td style={{padding:"7px 10px",fontWeight:500,whiteSpace:"nowrap"}}>
                    {r.resource}
                    {r.isSub&&<span style={{marginLeft:5,fontSize:10,padding:"1px 5px",borderRadius:3,background:"#FAEEDA",color:"#854F0B",fontWeight:600}}>SUB</span>}
                  </td>
                  <td style={{padding:"7px 10px",color:"#555"}}>{r.customer}</td>
                  <td style={{padding:"7px 10px",color:"#888780",fontFamily:"monospace",fontSize:11}}>{r.project}</td>
                  <td style={{padding:"7px 10px",color:"#555"}}>{r.milestone}</td>
                  <td style={{padding:"7px 10px"}}>{r.task}</td>
                  <td style={{padding:"7px 10px"}}>
                    <span style={{fontSize:11,padding:"2px 6px",borderRadius:4,
                      background:r.status==="Complete"?"#EAF3DE":"#f0ede8",
                      color:r.status==="Complete"?"#3B6D11":"#888780"}}>{r.status}</span>
                  </td>
                  <td style={{padding:"7px 10px"}}>
                    <span style={{fontSize:11,padding:"2px 6px",borderRadius:4,
                      background:r.payType==="unit"?"#fffbe6":"#f0ede8",
                      color:r.payType==="unit"?"#854F0B":"#555",fontWeight:500}}>{r.payType==="unit"?"Unit Rate":"Hourly"}</span>
                  </td>
                  <td style={{padding:"7px 10px",fontFamily:"monospace",fontSize:11}}>
                    {r.payType==="unit"?r.units.toLocaleString()+" "+r.uom:r.rdbHours.toFixed(1)+"h"}
                  </td>
                  <td style={{padding:"7px 10px",fontFamily:"monospace",fontSize:11}}>
                    {r.payType==="unit"?"$"+r.unitRate+"/"+r.uom:"$"+r.eeRate+"/hr"}
                  </td>
                  <td style={{padding:"7px 10px",fontWeight:600,color:r.invoiceable?"#1a1a18":"#aaa89e"}}>
                    ${Math.round(r.payAmount).toLocaleString()}
                  </td>
                </tr>
              ))}
            </tbody>
            <tfoot><tr style={{background:"#f8f7f4",borderTop:"1px solid #dddbd4"}}>
              <td colSpan={9} style={{padding:"8px 10px",fontWeight:600,fontSize:12}}>Total</td>
              <td style={{padding:"8px 10px",fontWeight:700,fontSize:13}}>${Math.round(filtered.reduce((s,r)=>s+r.payAmount,0)).toLocaleString()}</td>
            </tr></tfoot>
          </table>
        </>
      )}
